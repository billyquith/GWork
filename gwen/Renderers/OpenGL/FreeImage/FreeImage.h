...
#else // if FREEIMAGE_COLORORDER == FREEIMAGE_COLORORDER_BGR
...
#define FI_RGBA_ALPHA_SHIFT     24
#endif    // FREEIMAGE_COLORORDER
#else // ifndef FREEIMAGE_BIGENDIAN
#if FREEIMAGE_COLORORDER == FREEIMAGE_COLORORDER_BGR
...
FI_STRUCT(FIICCPROFILE)
{
    ...
    void*   data;   // points to a block of contiguous memory containing the
                    // profile
};
...
FI_ENUM(FREE_IMAGE_TYPE)
{
    ...
    FIT_BITMAP  = 1,    // standard image			: 1-, 4-, 8-, 16-, 24-,
                        // 32-bit
    FIT_UINT16  = 2,    // array of unsigned short	: unsigned 16-bit
    FIT_INT16  = 3,     // array of short			: signed 16-bit
    FIT_UINT32   = 4,       // array of unsigned long	: unsigned 32-bit
    FIT_INT32   = 5,        // array of long			: signed 32-bit
    FIT_FLOAT = 6,          // array of float			: 32-bit IEEE floating
                            // point
    FIT_DOUBLE  = 7,            // array of double			: 64-bit IEEE
                                // floating point
    FIT_COMPLEX    = 8,             // array of FICOMPLEX		: 2 x 64-bit
                                    // IEEE floating point
    FIT_RGB16  = 9,             // 48-bit RGB image			: 3 x 16-bit
    FIT_RGBA16   = 10,              // 64-bit RGBA image		: 4 x 16-bit
    FIT_RGBF    = 11,               // 96-bit RGB float image	: 3 x 32-bit
                                    // IEEE floating point
    FIT_RGBAF    = 12                   // 128-bit RGBA float image	: 4 x 32-bit
                                        // IEEE floating point
};
...
FI_ENUM(FREE_IMAGE_QUANTIZE)
{
    ...
    FIQ_NNQUANT = 1         // NeuQuant neural-net quantization algorithm by
                            // Anthony Dekker
};
...
FI_ENUM(FREE_IMAGE_DITHER)
{
    ...
    FID_BAYER4x4 = 1,           // Bayer ordered dispersed dot dithering (order
                                // 2 dithering matrix)
    FID_BAYER8x8  = 2,              // Bayer ordered dispersed dot dithering
                                    // (order 3 dithering matrix)
    FID_CLUSTER6x6 = 3,                 // Ordered clustered dot dithering
                                        // (order 3 - 6x6 matrix)
    FID_CLUSTER8x8    = 4,                  // Ordered clustered dot dithering
                                            // (order 4 - 8x8 matrix)
    FID_CLUSTER16x16 = 5,                   // Ordered clustered dot dithering
                                            // (order 8 - 16x16 matrix)
    FID_BAYER16x16   = 6                    // Bayer ordered dispersed dot
                                            // dithering (order 4 dithering
                                            // matrix)
};
...
FI_ENUM(FREE_IMAGE_JPEG_OPERATION)
{
    ...
    FIJPEG_OP_TRANSPOSE      = 3,                       // transpose across
                                                        // UL-to-LR axis
    FIJPEG_OP_TRANSVERSE  = 4,                          // transpose across
                                                        // UR-to-LL axis
    FIJPEG_OP_ROTATE_90        = 5,                             // 90-degree
                                                                // clockwise
                                                                // rotation
    FIJPEG_OP_ROTATE_180    = 6,                                // 180-degree
                                                                // rotation
    FIJPEG_OP_ROTATE_270 = 7                                    // 270-degree
                                                                // clockwise (or
                                                                // 90 ccw)
};
...
FI_ENUM(FREE_IMAGE_TMO)
{
    ...
    FITMO_FATTAL02     = 2      // Gradient domain high dynamic range
                                // compression (R. Fattal, 2002)
};
...
FI_ENUM(FREE_IMAGE_MDTYPE)
{
    ...
    FIDT_SLONG       = 9,                                   // 32-bit signed
                                                            // integer
    FIDT_SRATIONAL   = 10,                                      // 64-bit signed
                                                                // fraction
    FIDT_FLOAT      = 11,                                       // 32-bit IEEE
                                                                // floating
                                                                // point
    FIDT_DOUBLE     = 12,                                           // 64-bit
                                                                    // IEEE
                                                                    // floating
                                                                    // point
    FIDT_IFD     = 13,                                              // 32-bit
...
FI_ENUM(FREE_IMAGE_MDMODEL)
{
    ...
    FIMD_EXIF_MAKERNOTE = 4,                            // Exif maker note
                                                        // metadata
    FIMD_EXIF_INTEROP   = 5,                            // Exif interoperability
                                                        // metadata
    FIMD_IPTC         = 6,                              // IPTC/NAA metadata
    FIMD_XMP          = 7,                                      // Abobe XMP
                                                                // metadata
    FIMD_GEOTIFF     = 8,                                               //
                                                                        // GeoTIFF
                                                                        // metadata
...
typedef unsigned (DLL_CALLCONV*FI_ReadProc)(void* buffer, unsigned size, unsigned count,
                                            fi_handle handle);
typedef unsigned (DLL_CALLCONV*FI_WriteProc)(void* buffer, unsigned size, unsigned count,
                                             fi_handle handle);
...
typedef FIBITMAP* (DLL_CALLCONV*FI_LoadProc)(FreeImageIO* io, fi_handle handle, int page, int flags,
                                             void* data);
typedef BOOL (DLL_CALLCONV*FI_SaveProc)(FreeImageIO* io, FIBITMAP* dib, fi_handle handle, int page,
                                        int flags, void* data);
...
#define FIF_LOAD_NOPIXELS 0x8000 // loading: load the image header only (not
...
#define EXR_DEFAULT         0       // save data as half with piz-based wavelet
                                    // compression
#define EXR_FLOAT           0x0001  // save data as float instead of as half
                                    // (not recommended)
#define EXR_NONE            0x0002  // save with no compression
#define EXR_ZIP             0x0004  // save with zlib compression, in blocks of
                                    // 16 scan lines
...
#define EXR_B44             0x0020  // save with lossy 44% float compression -
                                    // goes to 22% when combined with EXR_LC
#define EXR_LC              0x0040  // save images with one luminance and two
                                    // chroma channels, rather than as RGB
                                    // (lossy compression)
#define FAXG3_DEFAULT       0
#define GIF_DEFAULT         0
#define GIF_LOAD256         1       // Load the image as a 256 color image with
                                    // ununsed palette entries, if it's 16 or 2
                                    // color
#define GIF_PLAYBACK        2       // 'Play' the GIF to generate each frame (as
                                    // 32bpp) instead of returning raw frame
                                    // data when loading
#define HDR_DEFAULT         0
#define ICO_DEFAULT         0
#define ICO_MAKEALPHA       1       // convert to 32bpp and create an alpha
...
#define JPEG_DEFAULT        0       // loading (see JPEG_FAST); saving (see
                                    // JPEG_QUALITYGOOD|JPEG_SUBSAMPLING_420)
#define JPEG_FAST           0x0001  // load the file as fast as possible,
                                    // sacrificing some quality
#define JPEG_ACCURATE       0x0002  // load the file with the best quality,
                                    // sacrificing some speed
#define JPEG_CMYK           0x0004  // load separated CMYK "as is" (use | to
                                    // combine with other load flags)
#define JPEG_EXIFROTATE     0x0008  // load and rotate according to Exif
...
#define JPEG_PROGRESSIVE    0x2000  // save as a progressive-JPEG (use | to
                                    // combine with other save flags)
#define JPEG_SUBSAMPLING_411 0x1000     // save with high 4x1 chroma subsampling
                                        // (4:1:1)
#define JPEG_SUBSAMPLING_420 0x4000     // save with medium 2x2 medium chroma
                                        // subsampling (4:2:0) - default value
#define JPEG_SUBSAMPLING_422 0x8000     // save with low 2x1 chroma subsampling
                                        // (4:2:2)
#define JPEG_SUBSAMPLING_444 0x10000    // save with no chroma subsampling
                                        // (4:4:4)
#define JPEG_OPTIMIZE       0x20000     // on saving, compute optimal Huffman
                                        // coding tables (can reduce a few
                                        // percent of file size)
#define JPEG_BASELINE       0x40000     // save basic JPEG, without metadata or
                                        // any markers
...
#define PNG_Z_BEST_SPEED            0x0001  // save using ZLib level 1
                                            // compression flag (default value
                                            // is 6)
#define PNG_Z_DEFAULT_COMPRESSION   0x0006  // save using ZLib level 6
                                            // compression flag (default
                                            // recommended value)
#define PNG_Z_BEST_COMPRESSION      0x0009  // save using ZLib level 9
                                            // compression flag (default value
                                            // is 6)
#define PNG_Z_NO_COMPRESSION        0x0100  // save without ZLib compression
#define PNG_INTERLACED              0x0200  // save using Adam7 interlacing (use
                                            // | to combine with other save
                                            // flags)
...
#define PSD_CMYK            1       // reads tags for separated CMYK (default is
                                    // conversion to RGB)
#define PSD_LAB             2       // reads tags for CIELab (default is
                                    // conversion to RGB)
#define RAS_DEFAULT         0
#define RAW_DEFAULT         0       // load the file as linear RGB 48-bit
#define RAW_PREVIEW         1       // try to load the embedded JPEG preview
                                    // with included Exif Data or default to RGB
                                    // 24-bit
...
#define TARGA_LOAD_RGB888   1       // If set the loader converts RGB555 and
                                    // ARGB8888 -> RGB888.
#define TARGA_SAVE_RLE      2       // If set, the writer saves with RLE
                                    // compression
#define TIFF_DEFAULT        0
#define TIFF_CMYK           0x0001  // reads/stores tags for separated CMYK (use
                                    // | to combine with compression flags)
...
#define FI_COLOR_IS_RGB_COLOR           0x00    // RGBQUAD color is a RGB color
                                                // (contains no valid alpha
                                                // channel)
#define FI_COLOR_IS_RGBA_COLOR          0x01    // RGBQUAD color is a RGBA color
                                                // (contains a valid alpha
                                                // channel)
#define FI_COLOR_FIND_EQUAL_COLOR       0x02    // For palettized images: lookup
                                                // equal RGB color from palette
#define FI_COLOR_ALPHA_IS_INDEX         0x04    // The color's rgbReserved
                                                // member (alpha) contains the
                                                // palette index to be used
...
typedef void (DLL_CALLCONV*FreeImage_OutputMessageFunctionStdCall)(FREE_IMAGE_FORMAT fif,
                                                                   const char* msg);
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Allocate(int width, int height, int bpp, unsigned red_mask FI_DEFAULT(
                                                      0), unsigned green_mask FI_DEFAULT(
                                                      0), unsigned blue_mask FI_DEFAULT(0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_AllocateT(FREE_IMAGE_TYPE type, int width, int height, int bpp FI_DEFAULT(
                                                       8), unsigned red_mask FI_DEFAULT(
                                                       0), unsigned green_mask FI_DEFAULT(
                                                       0), unsigned blue_mask FI_DEFAULT(0));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Load(FREE_IMAGE_FORMAT fif, const char* filename, int flags FI_DEFAULT(
                                                  0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_LoadU(FREE_IMAGE_FORMAT fif, const wchar_t* filename, int flags FI_DEFAULT(
                                                   0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_LoadFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO*io,
                                                        fi_handle handle,
                                                        int flags FI_DEFAULT(0));
DLL_API BOOL DLL_CALLCONV FreeImage_Save(FREE_IMAGE_FORMAT fif, FIBITMAP*dib, const char* filename, int flags FI_DEFAULT(
                                             0));
...
DLL_API BOOL DLL_CALLCONV FreeImage_SaveToHandle(FREE_IMAGE_FORMAT fif, FIBITMAP*dib,
                                                 FreeImageIO*io, fi_handle handle, int flags FI_DEFAULT(
                                                     0));

// Memory I/O stream routines -----------------------------------------------

DLL_API FIMEMORY* DLL_CALLCONV FreeImage_OpenMemory(BYTE*data FI_DEFAULT(
                                                        0), DWORD size_in_bytes FI_DEFAULT(0));
DLL_API void DLL_CALLCONV FreeImage_CloseMemory(FIMEMORY* stream);
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_LoadFromMemory(FREE_IMAGE_FORMAT fif, FIMEMORY*stream, int flags FI_DEFAULT(
                                                            0));
...
DLL_API BOOL DLL_CALLCONV FreeImage_AcquireMemory(FIMEMORY* stream, BYTE** data,
                                                  DWORD* size_in_bytes);
DLL_API unsigned DLL_CALLCONV FreeImage_ReadMemory(void* buffer, unsigned size, unsigned count,
                                                   FIMEMORY* stream);
DLL_API unsigned DLL_CALLCONV FreeImage_WriteMemory(const void* buffer, unsigned size,
                                                    unsigned count, FIMEMORY* stream);

DLL_API FIMULTIBITMAP* DLL_CALLCONV FreeImage_LoadMultiBitmapFromMemory(FREE_IMAGE_FORMAT fif,
                                                                        FIMEMORY*stream, int flags FI_DEFAULT(
                                                                            0));
DLL_API BOOL DLL_CALLCONV FreeImage_SaveMultiBitmapToMemory(FREE_IMAGE_FORMAT fif,
                                                            FIMULTIBITMAP* bitmap, FIMEMORY* stream,
                                                            int flags);
...
DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_RegisterLocalPlugin(FI_InitProc proc_address, const char* format FI_DEFAULT(
                                                                         0), const char* description FI_DEFAULT(
                                                                         0), const char* extension FI_DEFAULT(
                                                                         0), const char* regexpr FI_DEFAULT(
                                                                         0));
DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_RegisterExternalPlugin(const char* path, const char* format FI_DEFAULT(
                                                                            0), const char* description FI_DEFAULT(
                                                                            0), const char* extension FI_DEFAULT(
                                                                            0), const char* regexpr FI_DEFAULT(
                                                                            0));
...
DLL_API BOOL DLL_CALLCONV FreeImage_GetLockedPageNumbers(FIMULTIBITMAP* bitmap, int* pages,
                                                         int* count);

// Filetype request routines ------------------------------------------------

DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFileType(const char* filename, int size FI_DEFAULT(
                                                                 0));
DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFileTypeU(const wchar_t* filename, int size FI_DEFAULT(
                                                                  0));
DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFileTypeFromHandle(FreeImageIO*io,
                                                                       fi_handle handle, int size FI_DEFAULT(
                                                                           0));
DLL_API FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFileTypeFromMemory(FIMEMORY*stream, int size FI_DEFAULT(
                                                                           0));
...
DLL_API BOOL DLL_CALLCONV FreeImage_LookupX11Color(const char* szColor, BYTE* nRed, BYTE* nGreen,
                                                   BYTE* nBlue);
DLL_API BOOL DLL_CALLCONV FreeImage_LookupSVGColor(const char* szColor, BYTE* nRed, BYTE* nGreen,
                                                   BYTE* nBlue);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine1To4(BYTE* target, BYTE* source,
                                                    int width_in_pixels);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To4(BYTE* target, BYTE* source, int width_in_pixels,
                                                    RGBQUAD* palette);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To16_555(BYTE* target, BYTE* source,
                                                         int width_in_pixels, RGBQUAD* palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine16_565_To16_555(BYTE* target, BYTE* source,
                                                               int width_in_pixels);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To16_565(BYTE* target, BYTE* source,
                                                         int width_in_pixels, RGBQUAD* palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine16_555_To16_565(BYTE* target, BYTE* source,
                                                               int width_in_pixels);
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_ColorQuantizeEx(FIBITMAP*dib, FREE_IMAGE_QUANTIZE quantize FI_DEFAULT(
                                                             FIQ_WUQUANT), int PaletteSize FI_DEFAULT(
                                                             256), int ReserveSize FI_DEFAULT(
                                                             0), RGBQUAD*ReservePalette FI_DEFAULT(
                                                             NULL));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Threshold(FIBITMAP* dib, BYTE T);
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Dither(FIBITMAP* dib, FREE_IMAGE_DITHER algorithm);

DLL_API FIBITMAP* DLL_CALLCONV FreeImage_ConvertFromRawBits(BYTE*bits, int width, int height,
                                                            int pitch, unsigned bpp,
                                                            unsigned red_mask, unsigned green_mask,
                                                            unsigned blue_mask, BOOL topdown FI_DEFAULT(
                                                                FALSE));
DLL_API void DLL_CALLCONV FreeImage_ConvertToRawBits(BYTE*bits, FIBITMAP*dib, int pitch,
                                                     unsigned bpp, unsigned red_mask,
                                                     unsigned green_mask, unsigned blue_mask, BOOL topdown FI_DEFAULT(
                                                         FALSE));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_ConvertToStandardType(FIBITMAP*src, BOOL scale_linear FI_DEFAULT(
                                                                   TRUE));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_ConvertToType(FIBITMAP*src, FREE_IMAGE_TYPE dst_type, BOOL scale_linear FI_DEFAULT(
                                                           TRUE));

// tone mapping operators
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_ToneMapping(FIBITMAP*dib, FREE_IMAGE_TMO tmo, double first_param FI_DEFAULT(
                                                         0), double second_param FI_DEFAULT(0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_TmoDrago03(FIBITMAP*src, double gamma FI_DEFAULT(
                                                        2.2), double exposure FI_DEFAULT(0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_TmoReinhard05(FIBITMAP*src, double intensity FI_DEFAULT(
                                                           0), double contrast FI_DEFAULT(0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_TmoReinhard05Ex(FIBITMAP*src, double intensity FI_DEFAULT(
                                                             0), double contrast FI_DEFAULT(
                                                             0), double adaptation FI_DEFAULT(
                                                             1), double color_correction FI_DEFAULT(
                                                             0));

DLL_API FIBITMAP* DLL_CALLCONV FreeImage_TmoFattal02(FIBITMAP*src, double color_saturation FI_DEFAULT(
                                                         0.5),
                                                     double attenuation FI_DEFAULT(0.85));

// ZLib interface -----------------------------------------------------------

DLL_API DWORD DLL_CALLCONV FreeImage_ZLibCompress(BYTE* target, DWORD target_size, BYTE* source,
                                                  DWORD source_size);
DLL_API DWORD DLL_CALLCONV FreeImage_ZLibUncompress(BYTE* target, DWORD target_size, BYTE* source,
                                                    DWORD source_size);
DLL_API DWORD DLL_CALLCONV FreeImage_ZLibGZip(BYTE* target, DWORD target_size, BYTE* source,
                                              DWORD source_size);
DLL_API DWORD DLL_CALLCONV FreeImage_ZLibGUnzip(BYTE* target, DWORD target_size, BYTE* source,
                                                DWORD source_size);
...
DLL_API const char* DLL_CALLCONV FreeImage_TagToString(FREE_IMAGE_MDMODEL model, FITAG*tag, char* Make FI_DEFAULT(
                                                           NULL));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Rotate(FIBITMAP* dib, double angle, const void* bkcolor FI_DEFAULT(
                                                    NULL));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_RotateEx(FIBITMAP* dib, double angle, double x_shift,
                                                  double y_shift, double x_origin, double y_origin,
                                                  BOOL use_mask);
DLL_API BOOL DLL_CALLCONV FreeImage_FlipHorizontal(FIBITMAP* dib);
DLL_API BOOL DLL_CALLCONV FreeImage_FlipVertical(FIBITMAP* dib);
DLL_API BOOL DLL_CALLCONV FreeImage_JPEGTransform(const char* src_file, const char* dst_file,
                                                  FREE_IMAGE_JPEG_OPERATION operation, BOOL perfect FI_DEFAULT(
                                                      FALSE));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_MakeThumbnail(FIBITMAP*dib, int max_pixel_size, BOOL convert FI_DEFAULT(
                                                           TRUE));
...
DLL_API BOOL DLL_CALLCONV FreeImage_GetHistogram(FIBITMAP*dib, DWORD*histo, FREE_IMAGE_COLOR_CHANNEL channel FI_DEFAULT(
                                                     FICC_BLACK));
DLL_API int DLL_CALLCONV FreeImage_GetAdjustColorsLookupTable(BYTE* LUT, double brightness,
                                                              double contrast, double gamma,
                                                              BOOL invert);
DLL_API BOOL DLL_CALLCONV FreeImage_AdjustColors(FIBITMAP*dib, double brightness, double contrast,
                                                 double gamma, BOOL invert FI_DEFAULT(FALSE));
DLL_API unsigned DLL_CALLCONV FreeImage_ApplyColorMapping(FIBITMAP* dib, RGBQUAD* srccolors,
...
DLL_API unsigned DLL_CALLCONV FreeImage_ApplyPaletteIndexMapping(FIBITMAP* dib, BYTE* srcindices,
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Copy(FIBITMAP* dib, int left, int top, int right,
                                              int bottom);
DLL_API BOOL DLL_CALLCONV FreeImage_Paste(FIBITMAP* dst, FIBITMAP* src, int left, int top,
                                          int alpha);
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_Composite(FIBITMAP*fg, BOOL useFileBkg FI_DEFAULT(
                                                       FALSE), RGBQUAD*appBkColor FI_DEFAULT(
                                                       NULL), FIBITMAP*bg FI_DEFAULT(NULL));
DLL_API BOOL DLL_CALLCONV FreeImage_JPEGCrop(const char* src_file, const char* dst_file, int left,
                                             int top, int right, int bottom);
DLL_API BOOL DLL_CALLCONV FreeImage_JPEGCropU(const wchar_t* src_file, const wchar_t* dst_file,
                                              int left, int top, int right, int bottom);
DLL_API BOOL DLL_CALLCONV FreeImage_PreMultiplyWithAlpha(FIBITMAP* dib);

// background filling routines
DLL_API BOOL DLL_CALLCONV FreeImage_FillBackground(FIBITMAP*dib, const void* color, int options FI_DEFAULT(
                                                       0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_EnlargeCanvas(FIBITMAP*src, int left, int top, int right,
                                                       int bottom, const void* color, int options FI_DEFAULT(
                                                           0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_AllocateEx(int width, int height, int bpp,
                                                    const RGBQUAD*color, int options FI_DEFAULT(
                                                        0), const RGBQUAD*palette FI_DEFAULT(
                                                        NULL), unsigned red_mask FI_DEFAULT(
                                                        0), unsigned green_mask FI_DEFAULT(
                                                        0), unsigned blue_mask FI_DEFAULT(0));
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_AllocateExT(FREE_IMAGE_TYPE type, int width, int height,
                                                     int bpp, const void* color, int options FI_DEFAULT(
                                                         0), const RGBQUAD*palette FI_DEFAULT(
                                                         NULL), unsigned red_mask FI_DEFAULT(
                                                         0), unsigned green_mask FI_DEFAULT(
                                                         0), unsigned blue_mask FI_DEFAULT(0));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_MultigridPoissonSolver(FIBITMAP*Laplacian, int ncycle FI_DEFAULT(
                                                                    3));
