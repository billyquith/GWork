    ...
    namespace Skin
    {
        ...
        public:
                                      ...
                                      int textWidth)
            {
                ...
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+1+textStart+textWidth, rect.y+1, rect.w-
                                                     textStart+textWidth-2, 1) );
                ...
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+1+textStart+textWidth, rect.y, rect.w-
                                                     textStart-textWidth-2, 1) );
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+1, (rect.y+rect.h)-1, rect.w-2, 1) );
            ...
            virtual void DrawWindow(Gwen::Controls::Base* control, int topHeight, bool inFocus)
            {
                ...
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+1, rect.y+topHeight-1, iBorderSize,
                                                     rect.h-2-topHeight) );
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+rect.w-iBorderSize, rect.y+topHeight-1,
                                                     iBorderSize, rect.h-2-topHeight) );
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+1, rect.y+rect.h-iBorderSize, rect.w-2,
                                                     iBorderSize) );
                // Main inner
                m_Render->SetDrawColor( Gwen::Color(m_colControlDark.r, m_colControlDark.g,
                                                    m_colControlDark.b, 230) );
                m_Render->DrawFilledRect( Gwen::Rect(rect.x+iBorderSize+1, rect.y+topHeight, rect.w-
                                                     iBorderSize*2-2, rect.h-topHeight-iBorderSize-
                                                     1) );
                // Light inner border
                m_Render->SetDrawColor( Gwen::Color(255, 255, 255, 100) );
                m_Render->DrawShavedCornerRect( Gwen::Rect(rect.x+1, rect.y+1, rect.w-2, rect.h-
                                                           2) );
                // Dark line between titlebar and main
                m_Render->SetDrawColor(m_colBorderColor);
                // Inside border
                m_Render->SetDrawColor(m_colControlOutlineNormal);
                m_Render->DrawLinedRect( Gwen::Rect( rect.x+iBorderSize, rect.y+topHeight-1, rect.w-
                                                     10, rect.h-topHeight-(iBorderSize-1) ) );
                // Dark outer border
                m_Render->SetDrawColor(m_colBorderColor);
                                         ...
                                         float progress)
            {
                ...
                else
                {
                    ...
                    // Top half
                    m_Render->SetDrawColor(FillColour);
                    m_Render->DrawFilledRect( Gwen::Rect(1, 1+( rect.h*(1-progress) ), rect.w-2,
                                                         rect.h*progress-2) );
                    m_Render->SetDrawColor( Gwen::Color(255, 255, 255, 150) );
            ...
            {
                // DrawButton( control->Width(), control->Height(), bDepressed,
                // false, true );
                m_Render->SetDrawColor( Gwen::Color(0, 0, 0, 240) );
                Gwen::Rect r(control->Width()/2-2, control->Height()/2-2, 5, 5);
                DrawArrowDown(r);
            ...
            virtual void DrawTreeButton(Controls::Base* control, bool bOpen)
            {
                ...
                if (!bOpen)       // ! because the button shows intention, not
                                  // the current state
                {
                    m_Render->DrawFilledRect( Gwen::Rect(rect.x+rect.w/2,    rect.y+2,
                                                         1,
                                                         rect.h-4) );
                }

                m_Render->DrawFilledRect( Gwen::Rect(rect.x+2,           rect.y+rect.h/2,
                                                     rect.w-4, 1) );
            }
